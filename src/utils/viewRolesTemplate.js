const fallback = ['roleName', 'createdDateTime', 'updatedDateTime', 'actions'];
import { extractDateTime } from './extractDateTime';
import { camelCaseToSentenceCase } from './camelCaseToSentenceCase';

export const viewRolesTemplate = (data, selectedColumns = fallback) => {
    let head = [];
    let body = [];
    let allColumns = [];
    ...
    if(data && data.length>0) {
        ...
        data.map( (cols, i)=> {
            ...
            cells.push({ 'value': '', name: 'edit', 'items':[{ 'label': '', 'name': 'edit', 'link':'EDIT_ROLE', id: `${cols['manageRolesId']}` }], 'class': `view grid-cell grid-link row ${className}` });
            body.push(cells);   
        });
