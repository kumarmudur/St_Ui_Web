...
const fallback = ['purchaseOrderNo', 'supplier', 'supplierContact', 'status'];
export const managePurchaseOrderTemplate = (data, selectedColumns = fallback) => {
    let head = [];
    let body = [];
    ...
    //if(data && data.length>0) {
      ...
      save('manage-purchase-order', selectedColumns);
        ...
        data.map( (cols, i)=> {
            ...
            cells.push({ 'value': '', 'items':[
                { 'label': '', 'name': 'edit', 'link': 'EDIT_PURCHASE_ORDER', id: `${cols['purchaseOrderId']}` },
            ],
