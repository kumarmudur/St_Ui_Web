  ...
  componentWillReceiveProps(nextProps) {
    ...
    let preSelectedFilter =  loadData('manage-supplier');
        ...
        if(preSelectedFilter && preSelectedFilter.length>0) {
          const hideBox = true;
          this._filter(preSelectedFilter, hideBox, nextProps.supplierData);
        }
...
_filter = (checkedBoxes, hideBox, newData) => {
  ...
  save('manage-supplier', checkedBoxes);
  const gridTemplate = data && data.length>0 ? manageSupplierTemplate(data, checkedBoxes) : { 'head': [], 'body': [] };
  this.setState({
...
_clickHandler = (link, id) => {
  if ( link === 'EDIT_SUPPLIER' ) {
      this.props.editSupplier(link, id);
  } else if ( link === 'DELETE_SUPPLIER' ) {
      this.props.deleteSupplier(id);
  } else if (link === 'VIEW_SUPPLIER') {
    this.props.viewSupplier(link, id);
  }
}
      ...
      <div className="container">
       {
         ...
         { showFilterBox && <Filter page="manage-supplier"  allColumns={ allColumns }  _filterBoxToggle={ this._filterBoxToggle } _filter={ this._filter } /> }
         {
          recordsCount ? <Fragment>
          <Grid 
            head={ head }
...
ManageSupplierComponent.defaultProps = {
  template: {
      head: [],
      body:[]
  }
...
ManageSupplierComponent.propTypes = {
