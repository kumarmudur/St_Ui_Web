...
class ManagePurchasePlanComponent extends Component {
    ...
    constructor(props) {
      ...
      componentWillReceiveProps(nextProps) {
        ...
        this.setState({
            ...
            data: nextProps.purchasePlanData,
        ...
        if(preSelectedFilter && preSelectedFilter.length>0) {
          const hideBox = true;
          this._filter(preSelectedFilter, hideBox, nextProps.purchasePlanData);
        }
     }
    ...
    _filter = (checkedBoxes, hideBox, newData) => {
      ...
      save('purchase-plans', checkedBoxes);
      const gridTemplate = data && data.length > 0 ? managePurchasePlanTemplate(data, checkedBoxes) : { 'head': [], 'body': [] };
      this.setState({
    ...
    _clickHandler = (link, id) => {
      ...
      if ( link === 'EDIT_PURCHASE_PLAN' ) {
          this.props.editPurchasePlan(link, id);
      } else if ( link === 'DELETE_PURCHASE_PLAN' ) {
          this.props.deletePurchasePlan(id, pageSize);
      } else if( link === 'VIEW_PURCHASE_PLAN' ) {
        this.props.viewPurchasePlan(link, id);
      }
    }
    ...
    render() {
        const { template, showFilterUsersBox, recordsCount, activePage, pageSize, recordsMin, recordsMax } = this.state;
        ...
        { showFilterUsersBox && <Filter page="purchase-plans"  allColumns={ allColumns }  _filterBoxToggle={ this._filterBoxToggle } _filter={ this._filter } /> }
        {
          recordsCount ? <Fragment>
          <Grid 
            head={ head }
...
ManagePurchasePlanComponent.defaultProps = {
    template: {
        head: [],
        body:[]
    }
  ...
  ManagePurchasePlanComponent.propTypes = {
